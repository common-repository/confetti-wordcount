 ...
 * complete: {Function} complete complete function
 *
 *	@class confetti
 */
...
$.fn.confetti = function (options) {
	...
	exception(c.airResistanceNum,c.gravityNum,c.intervalSpeed,c.complete);
	...
	function set(x,y,num,speedRange) {
		for (var i = 0; i < num; i++) {
			$('#wp-content-wrap').append('<div class="' + className + i + '"></div>');
			random[i] = {};
	 ...
	 */
	function exception(airResistanceNum,gravityNum,intervalSpeed,complete){
	 ...
	 * @param  {Function} complete complete function
	 */
	function gravity($confetti,speedX,speedY,airResistanceNum,gravityNum,intervalSpeed,complete){
    	...
    	speedY = speedY * airResistanceNum + gravityNum;
			...
			if($('#wp-content-wrap').height() < topPosition + speedY) {
				removeNum++;
				$confetti.remove();
			};
